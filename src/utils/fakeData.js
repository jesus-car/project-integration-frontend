export const properties = [
    {
        id: 1,
        name: "Casa de playa privada y de lujo con piscina infinita",
        pricePerNight: 100000,
        description: "Bienvenido a Casa Arissa, una hermosa y exclusiva villa frente a la playa ubicada en el sector de El Francés, Playa Guacamaya, la parte más privada y exclusiva de la zona.",
        city: "Santa Marta",
        country: "Colombia",
        category: "playa",
        images: [
            'https://a0.muscache.com/im/pictures/miso/Hosting-1231218421417858848/original/43117236-c7d2-40c2-a70c-4b141d8171c5.jpeg?im_w=1200',
        ]
    },
    {
        id: 2,
        name: "Casa de playa con piscina privada",
        pricePerNight: 150000,
        description: "Casa de playa con piscina privada, ubicada en la exclusiva zona de Bello Horizonte, a solo 10 minutos del aeropuerto y a 15 minutos del centro histórico de Santa Marta.",
        city: "Bariloche",
        country: "Argentina",
        category: "playa",
        images: [
            'https://a0.muscache.com/im/pictures/hosting/Hosting-1212273910034900968/original/8078283b-c8d2-4c00-ba15-cfd2bf0be27b.jpeg?im_w=1200',
        ]
    },
    {
        id: 3,
        name: "Casa de playa MULÉ + piscina",
        pricePerNight: 200000,
        description: "Casa Mulé está diseñada para combinar la naturaleza, el lujo y el confort ofreciendo una experiencia única, desde las impresionantes puestas de sol hasta la más relajante brisa del océano.",
        city: "Santiago de Tolú",
        country: "Colombia",
        category: "playa",
        images: [
            'https://a0.muscache.com/im/pictures/miso/Hosting-1003548971445736385/original/3a46cf50-b38c-488a-81ce-69f270f46eb3.jpeg?im_w=1200',
        ]
    },
    {
        id: 4,
        name: "Frente a la Playa con Descuento | Lujo y Estilo",
        pricePerNight: 100000,
        description: "Impecable mansión frente al mar de 5 dorm., ubicada directamente sobre la arena con impresionantes vistas al mar. Diseñado con elegancia, este retiro exclusivo promete una escapada inolvidable junto al mar.",
        city: "El Chirú",
        country: "Panamá",
        category: "playa",
        images: [
            'https://a0.muscache.com/im/pictures/miso/Hosting-1122694246457285445/original/d4407c43-6f2b-453b-8cc4-303734ac61a6.jpeg?im_w=1200',
        ]
    },
    {
        id: 5,
        name: "Glamping con desayuno y jacuzzi!",
        pricePerNight: 230000,
        description: "¡Vive la experiencia de Glamping en nuestro espacio único! Disfruta de la naturaleza y la tranquilidad en un entorno seguro y cómodo.",
        city: "San Pedro de Atacama",
        country: "Chile",
        category: "montaña",
        images: [
            'https://a0.muscache.com/im/pictures/miso/Hosting-1157222746294729818/original/f5586a11-c147-4fd1-9895-cb00b36eee10.jpeg?im_w=1200',
        ]
    },
    {
        id: 6,
        name: "Hermosa villa con piscina y vista a las montañas",
        pricePerNight: 150000,
        description: "Hermosa villa con piscina y vista a las montañas, ubicada en la exclusiva zona de Bello Horizonte, a solo 10 minutos del aeropuerto y a 15 minutos del centro histórico de Santa Marta.",
        city: "Bariloche",
        country: "Argentina",
        category: "montaña",
        images: [
            'https://a0.muscache.com/im/pictures/airflow/Hosting-892813186472291719/original/44244a51-7e71-4a55-b8a2-fa860ce8b948.jpg?im_w=1200',
        ]
    },
    {
        id: 7,
        name: "Cabaña rústica en medio del bosque",
        pricePerNight: 80000,
        description: "Escapa de la rutina en esta cabaña rústica rodeada de naturaleza en el corazón del bosque.",
        city: "Villa La Angostura",
        country: "Argentina",
        category: "campo",
        images: [
            'https://a0.muscache.com/im/pictures/2aae9259-4a19-47c8-a7ba-6f146e226db6.jpg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 8,
        name: "Apartamento moderno con vista a la ciudad",
        pricePerNight: 120000,
        description: "Disfruta de las mejores vistas de la ciudad en este apartamento moderno y acogedor.",
        city: "Buenos Aires",
        country: "Argentina",
        category: "ciudad",
        images: [
            'https://a0.muscache.com/im/pictures/miso/Hosting-51978429/original/cb9b3d99-7cae-48ad-9000-37afe1e81769.jpeg?im_w=720',
        ]
    },
    {
        id: 9,
        name: "Villa de montaña con jacuzzi y chimenea",
        pricePerNight: 180000,
        description: "Una experiencia única en la montaña, ideal para relajarse junto al jacuzzi o la chimenea.",
        city: "San Martín de los Andes",
        country: "Argentina",
        category: "montaña",
        images: [
            'https://a0.muscache.com/im/pictures/94fc8327-fa4e-48e6-a802-f786614d9c51.jpg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 10,
        name: "Casa en la playa con vista al atardecer",
        pricePerNight: 90000,
        description: "Hermosa casa frente al mar donde podrás disfrutar de atardeceres espectaculares.",
        city: "Cancún",
        country: "México",
        category: "playa",
        images: [
            'https://a0.muscache.com/im/pictures/hosting/Hosting-955858690980318819/original/3740d4df-39e8-4a30-b9dd-5d17baec7a9d.jpeg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 11,
        name: "Rancho campestre con caballos y senderos",
        pricePerNight: 110000,
        description: "Vive una experiencia rural con caballos, senderos y mucho más en este rancho campestre.",
        city: "Medellín",
        country: "Colombia",
        category: "campo",
        images: [
            'https://a0.muscache.com/im/pictures/miso/Hosting-746210844868185359/original/fd07e82b-4a64-46de-a800-8bebe88f4822.jpeg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 12,
        name: "Penthouse lujoso con piscina privada",
        pricePerNight: 250000,
        description: "Penthouse exclusivo con piscina privada y vista panorámica de la ciudad.",
        city: "Nueva York",
        country: "Estados Unidos",
        category: "ciudad",
        images: [
            'https://a0.muscache.com/im/pictures/miso/Hosting-897701630996078259/original/71d40723-bc60-4fc3-ba88-79fba8ce8bee.jpeg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 13,
        name: "Glamping en la montaña con jacuzzi",
        pricePerNight: 200000,
        description: "Disfruta de la tranquilidad de la montaña en un espacio de glamping con jacuzzi privado.",
        city: "Cusco",
        country: "Perú",
        category: "montaña",
        images: [
            'https://a0.muscache.com/im/pictures/b79f133a-1a73-4499-a43e-9163b52cea33.jpg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 14,
        name: "Casa de campo con huerto orgánico",
        pricePerNight: 70000,
        description: "Casa de campo acogedora con huerto orgánico y vistas a las colinas.",
        city: "Tolima",
        country: "Colombia",
        category: "campo",
        images: [
            'https://a0.muscache.com/im/pictures/42c45e05-c866-4017-9045-f6c34aa27410.jpg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 15,
        name: "Villa de lujo en la playa con chef privado",
        pricePerNight: 300000,
        description: "Vive una experiencia de lujo en esta villa con chef privado y acceso directo a la playa.",
        city: "Miami",
        country: "Estados Unidos",
        category: "playa",
        images: [
            'https://a0.muscache.com/im/pictures/e63f53b6-0953-4d85-998a-16d4baa373cc.jpg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 16,
        name: "Cabaña rústica con piscina de agua natural",
        pricePerNight: 95000,
        description: "Cabaña en el campo con piscina de agua natural, ideal para desconectar.",
        city: "Monteverde",
        country: "Costa Rica",
        category: "campo",
        images: [
            'https://a0.muscache.com/im/pictures/43a6ee79-6685-4c3f-bb4d-8db164406006.jpg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 17,
        name: "Apartamento céntrico con balcón",
        pricePerNight: 85000,
        description: "Apartamento moderno en el centro de la ciudad con balcón y excelente ubicación.",
        city: "Lima",
        country: "Perú",
        category: "ciudad",
        images: [
            'https://a0.muscache.com/im/pictures/fa0dbacc-58e1-45ae-be91-da046333b08c.jpg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 18,
        name: "Refugio en las montañas con vistas espectaculares",
        pricePerNight: 130000,
        description: "Refugio de montaña con vistas impresionantes y rutas de senderismo cercanas.",
        city: "La Paz",
        country: "Bolivia",
        category: "montaña",
        images: [
            'https://a0.muscache.com/im/pictures/68e7546d-539c-463c-aed1-0cc8a7c0a46c.jpg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 19,
        name: "Villa en la playa con acceso privado al mar",
        pricePerNight: 160000,
        description: "Villa exclusiva con acceso privado al mar y espacios de entretenimiento.",
        city: "Punta Cana",
        country: "República Dominicana",
        category: "playa",
        images: [
            'https://a0.muscache.com/im/pictures/hosting/Hosting-U3RheVN1cHBseUxpc3Rpbmc6MzExODU2NjM%3D/original/d2a650e6-3386-45f5-a6b4-e5fb7a2449aa.jpeg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 20,
        name: "Casa en la ciudad con piscina y gimnasio",
        pricePerNight: 180000,
        description: "Moderna casa en la ciudad con piscina privada y gimnasio completo.",
        city: "Madrid",
        country: "España",
        category: "ciudad",
        images: [
            'https://a0.muscache.com/im/pictures/hosting/Hosting-1196141205624875008/original/841a7fc0-bd9e-4b9b-869c-0c268efafcae.jpeg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 21,
        name: "Ranchito en el campo con caballos y huerta",
        pricePerNight: 120000,
        description: "Perfecto para los amantes de la vida de campo, esta finca cuenta con una huerta, establos y caballos para paseos guiados.",
        city: "Villa de Leyva",
        country: "Colombia",
        category: "campo",
        images: [
            'https://a0.muscache.com/im/pictures/miso/Hosting-1055711668239141400/original/7db34c62-47f7-4de8-b4f9-085d1cbe7628.jpeg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 22,
        name: "Penthouse con vistas a la ciudad y jacuzzi",
        pricePerNight: 210000,
        description: "Experiencia de lujo en un penthouse en pleno centro de la ciudad, con jacuzzi y vistas panorámicas.",
        city: "Lima",
        country: "Perú",
        category: "ciudad",
        images: [
            'https://a0.muscache.com/im/pictures/36b1e51b-27fa-47be-9526-df2c2d33142a.jpg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 23,
        name: "Casita campestre con senderos naturales",
        pricePerNight: 95000,
        description: "Rodeada de naturaleza, esta casa campestre ofrece senderos privados y vistas de montañas cercanas, perfecta para descansar.",
        city: "Pucón",
        country: "Chile",
        category: "campo",
        images: [
            'https://a0.muscache.com/im/pictures/e41ce817-84d8-46b8-9b7b-9f6077c6d31a.jpg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 24,
        name: "Loft moderno con terraza y vista urbana",
        pricePerNight: 130000,
        description: "Disfruta de un loft moderno en pleno centro, con una terraza perfecta para apreciar el bullicio de la ciudad.",
        city: "Buenos Aires",
        country: "Argentina",
        category: "ciudad",
        images: [
            'https://a0.muscache.com/im/pictures/prohost-api/Hosting-1102255664744611948/original/50538cbd-9418-4b61-bbda-15f31a307690.jpeg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 25,
        name: "Cabaña en la montaña con chimenea",
        pricePerNight: 110000,
        description: "Rústica cabaña en medio de la montaña, equipada con una chimenea y senderos cercanos para caminatas.",
        city: "Ushuaia",
        country: "Argentina",
        category: "montaña",
        images: [
            'https://a0.muscache.com/im/pictures/prohost-api/Hosting-40799878/original/96e84ae8-bda4-498d-b35a-151c6b2567b5.jpeg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 26,
        name: "Suite con vistas al río y jacuzzi exterior",
        pricePerNight: 175000,
        description: "Disfruta de un jacuzzi exterior con vistas al río, en una suite que combina lujo y naturaleza.",
        city: "Puerto Iguazú",
        country: "Argentina",
        category: "ciudad",
        images: [
            'https://a0.muscache.com/im/pictures/miso/Hosting-817250467699332015/original/4f5bbf35-5232-499e-991f-4bb9ddb80e02.jpeg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 27,
        name: "Refugio playero con terraza y fogón",
        pricePerNight: 95000,
        description: "A solo unos metros de la playa, esta casa ofrece una terraza con fogón para pasar noches acogedoras bajo las estrellas.",
        city: "Punta del Este",
        country: "Uruguay",
        category: "playa",
        images: [
            'https://a0.muscache.com/im/pictures/miso/Hosting-1037390972405688567/original/0f4e4f76-1b02-4612-8b1f-f114201b96c4.jpeg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 28,
        name: "Cabaña rústica con piscina natural",
        pricePerNight: 185000,
        description: "Esta cabaña combina el estilo rústico con una piscina natural rodeada de vegetación exuberante.",
        city: "Pucallpa",
        country: "Perú",
        category: "campo",
        images: [
            'https://a0.muscache.com/im/pictures/miso/Hosting-52329661/original/800aa635-16b7-4ceb-adbc-3d2445edd9dd.jpeg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 29,
        name: "Villa con piscina infinita y vista al mar",
        pricePerNight: 260000,
        description: "Moderna villa ubicada en lo alto de un acantilado con una piscina infinita que se funde con el mar.",
        city: "Valparaíso",
        country: "Chile",
        category: "playa",
        images: [
            'https://a0.muscache.com/im/pictures/2018d69d-7036-4af5-ab11-937b674f5240.jpg?im_w=1440&im_q=highq',
        ]
    },
    {
        id: 30,
        name: "Eco-lodge en la selva con guía turístico",
        pricePerNight: 210000,
        description: "Experiencia eco-friendly en la selva con alojamiento de lujo y guía para explorar la flora y fauna locales.",
        city: "Iquitos",
        country: "Perú",
        category: "campo",
        images: [
            'https://a0.muscache.com/im/pictures/8b44aa4b-8f85-468c-ab39-15cef1581586.jpg?im_w=1440&im_q=highq',
        ]
    },


];

export const detail = {

    title: "Linda casa en la montaña",
    imgPrincipal:"../../public/images/casa1.3.jpeg",
    img: [
        "https://a0.muscache.com/im/pictures/miso/Hosting-856575418570950976/original/0aaefcd2-395c-45c8-bb6d-a57275e26e55.jpeg?im_w=1440",
        "https://a0.muscache.com/im/pictures/miso/Hosting-856575418570950976/original/f312b1eb-7064-4467-a18e-3f7fa7148ba5.jpeg?im_w=1440",
        "https://a0.muscache.com/im/pictures/miso/Hosting-856575418570950976/original/6720ad38-b5e8-4a08-8f7e-b1ed9f61b6cf.jpeg?im_w=1440",
        "https://a0.muscache.com/im/pictures/6d4b1ac9-b30c-4d53-a84f-c2dab0d04cf5.jpg?im_w=1440"
    ],
    allImg:[
        "https://a0.muscache.com/im/pictures/miso/Hosting-856575418570950976/original/0aaefcd2-395c-45c8-bb6d-a57275e26e55.jpeg?im_w=1440",
        "https://a0.muscache.com/im/pictures/miso/Hosting-856575418570950976/original/f312b1eb-7064-4467-a18e-3f7fa7148ba5.jpeg?im_w=1440",
        "https://a0.muscache.com/im/pictures/miso/Hosting-856575418570950976/original/6720ad38-b5e8-4a08-8f7e-b1ed9f61b6cf.jpeg?im_w=1440",
        "https://a0.muscache.com/im/pictures/6d4b1ac9-b30c-4d53-a84f-c2dab0d04cf5.jpg?im_w=1440"
    ],
    city: "Peñol",
    country: "Colombia",
    price: "294,32",
    guests: 6,
    bebrooms: 3,
    bebs: 6,
    bathrooms: 2,
    review: 4.89,
    host: "William Lee",
    yearhost: 6,
    workarea: true,
    description: "Relajate con toda la familia en este tranquilo alojamiento. " +
                    "La finca tiene la casa principal 🏠 con 3 dormitorios y 3 baños completos. " +
                    "Una cocina increíble para que la familia esté cerca y una chimenea de leña. " +
                    " 👪 La casa tiene una cama tamaño king, cocina completa, baño completo y una bonita chimenea de madera. " +
                    "La granja está rodeada de grandes árboles y vistas increíbles.",

    moreDescription: "Servicio de limpieza diario esta incluido. " +
                    "Por favor confirmar con anticipación la hora en la que les gustaría que la empleada entre a hacer el aseo.",

    registrerNumber: "102030",
    
    mountainView: true,
    securityCamera: true,
    freeParking: true,
    kitchen: true,
    petAllowed: true,
    wifi: true,
    tv: true,

};

export const users = [
    {
        "id": 1,
        "firstName": "Juan Pablo",
        "lastName": "Gómez Pérez",
        "email": "juanp@mail.com",
        "role": {
            "id": 3,
            "name": "ROLE_OWNER",
            "description": "Propietario",
            "permissions": [
                {
                    "id": 1,
                    "name": "READ_PROPERTIES"
                }
            ]
        }
    },
    {
        "id": 2,
        "firstName": "Ana María",
        "lastName": "López Fernández",
        "email": "anamaria@mail.com",
        "role": {
            "id": 2,
            "name": "ROLE_USER",
            "description": "Usuario",
            "permissions": [
                {
                    "id": 2,
                    "name": "READ_PROPERTIES"
                },
                {
                    "id": 3,
                    "name": "UPDATE_PROFILE"
                }
            ]
        }
    },
    {
        "id": 3,
        "firstName": "Carlos Alberto",
        "lastName": "Martínez Ruiz",
        "email": "carlosm@mail.com",
        "role": {
            "id": 1,
            "name": "ROLE_ADMIN",
            "description": "Administrador",
            "permissions": [
                {
                    "id": 4,
                    "name": "READ_PROPERTIES"
                },
                {
                    "id": 5,
                    "name": "WRITE_PROPERTIES"
                },
                {
                    "id": 6,
                    "name": "MANAGE_USERS"
                }
            ]
        }
    },
    {
        "id": 4,
        "firstName": "Marta Isabel",
        "lastName": "Sánchez Orozco",
        "email": "marta@mail.com",
        "role": {
            "id": 3,
            "name": "ROLE_OWNER",
            "description": "Propietario",
            "permissions": [
                {
                    "id": 1,
                    "name": "READ_PROPERTIES"
                }
            ]
        }
    },
    {
        "id": 5,
        "firstName": "José Luis",
        "lastName": "Hernández Silva",
        "email": "joseluis@mail.com",
        "role": {
            "id": 2,
            "name": "ROLE_USER",
            "description": "Usuario",
            "permissions": [
                {
                    "id": 2,
                    "name": "READ_PROPERTIES"
                },
                {
                    "id": 3,
                    "name": "UPDATE_PROFILE"
                }
            ]
        }
    },
    {
        "id": 6,
        "firstName": "Lucía",
        "lastName": "Rodríguez Muñoz",
        "email": "luciar@mail.com",
        "role": {
            "id": 1,
            "name": "ROLE_ADMIN",
            "description": "Administrador",
            "permissions": [
                {
                    "id": 4,
                    "name": "READ_PROPERTIES"
                },
                {
                    "id": 5,
                    "name": "WRITE_PROPERTIES"
                },
                {
                    "id": 6,
                    "name": "MANAGE_USERS"
                }
            ]
        }
    },
    {
        "id": 7,
        "firstName": "Fernando",
        "lastName": "García López",
        "email": "fernando@mail.com",
        "role": {
            "id": 3,
            "name": "ROLE_OWNER",
            "description": "Propietario",
            "permissions": [
                {
                    "id": 1,
                    "name": "READ_PROPERTIES"
                }
            ]
        }
    },
    {
        "id": 8,
        "firstName": "Patricia",
        "lastName": "Torres Ramírez",
        "email": "patricia@mail.com",
        "role": {
            "id": 2,
            "name": "ROLE_USER",
            "description": "Usuario",
            "permissions": [
                {
                    "id": 2,
                    "name": "READ_PROPERTIES"
                },
                {
                    "id": 3,
                    "name": "UPDATE_PROFILE"
                }
            ]
        }
    },
    {
        "id": 9,
        "firstName": "Raúl",
        "lastName": "González Pérez",
        "email": "raul@mail.com",
        "role": {
            "id": 1,
            "name": "ROLE_ADMIN",
            "description": "Administrador",
            "permissions": [
                {
                    "id": 4,
                    "name": "READ_PROPERTIES"
                },
                {
                    "id": 5,
                    "name": "WRITE_PROPERTIES"
                },
                {
                    "id": 6,
                    "name": "MANAGE_USERS"
                },
                {
                    "id": 7,
                    "name": "DELETE_PROPERTIES"
                }
            ]
        }
    },
    {
        "id": 10,
        "firstName": "Laura",
        "lastName": "Ramírez Torres",
        "email": "laura@mail.com",
        "role": {
            "id": 2,
            "name": "ROLE_USER",
            "description": "Usuario",
            "permissions": [
                {
                    "id": 2,
                    "name": "READ_PROPERTIES"
                },
                {
                    "id": 3,
                    "name": "UPDATE_PROFILE"
                },
                {
                    "id": 8,
                    "name": "VIEW_ANALYTICS"
                }
            ]
        }
    }
];
